...
public class T8107QueryTerminalPropertyExample {
    ...
    private static final Logger logger = LoggerFactory.getLogger(T8107QueryTerminalPropertyExample.class);

    public static void main(String[] args) {
        logger.info("=== T8107查询终端属性消息示例 ===");
    ...
    private static void demonstrateConstructorCreation() {
        ...
        logger.info("   消息体长度: " + message.encodeBody().length());
        logger.info("   消息描述: " + message.toString());
    }
    ...
    private static void demonstrateFactoryCreation() {
        ...
        logger.info("   消息ID验证: " + (message.getMessageId() == 0x8107 ? "通过" : "失败"));
        logger.info("   是否为T8107实例: " + (message instanceof T8107QueryTerminalProperty ? "是" : "否"));
    }
    ...
    private static void demonstrateEncodeDecode() {
        ...
        logger.info("   编码后的消息体长度: " + encoded.length() + " 字节");
        logger.info("   编码后的消息体内容: " + (encoded.length() == 0 ? "空" : "非空"));
        
        // 解码消息体
        T8107QueryTerminalProperty decoded = new T8107QueryTerminalProperty();
        decoded.decodeBody(encoded);
        
        logger.info("   解码后的消息ID: 0x" + Integer.toHexString(decoded.getMessageId()).toUpperCase());
        logger.info("   编解码一致性: " + (original.getMessageId() == decoded.getMessageId() ? "通过" : "失败"));
    }
    ...
    private static void demonstrateCompleteFlow() {
        ...
        try {
            ...
            logger.info("   完整消息编码长度: " + encodedMessage.length() + " 字节");
            logger.info("   消息开始标志: 0x" + Integer.toHexString(encodedMessage.getByte(0) & 0xFF).toUpperCase());
            logger.info("   消息结束标志: 0x" + Integer.toHexString(encodedMessage.getByte(encodedMessage.length() - 1) & 0xFF).toUpperCase());
            
            // 解码完整消息
            JT808Decoder decoder = new JT808Decoder();
            JT808Message decodedMessage = decoder.decode(encodedMessage);
            
            logger.info("   解码后的消息类型: " + decodedMessage.getClass().getSimpleName());
            logger.info("   解码后的消息ID: 0x" + Integer.toHexString(decodedMessage.getMessageId()).toUpperCase());
            logger.info("   解码后的手机号: " + decodedMessage.getHeader().getPhoneNumber());
            logger.info("   解码后的流水号: " + decodedMessage.getHeader().getSerialNumber());
            ...
            if (decodedMessage instanceof T8107QueryTerminalProperty) {
                ...
                logger.info("   类型转换: 成功");
                logger.info("   消息描述: " + t8107.toString());
            } else {
                logger.info("   类型转换: 失败");
            }
