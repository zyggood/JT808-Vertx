 ...
 * @since 1.0.0
 */
class T0A00TerminalRSAPublicKeyTest {
    
    @Test
    void testMessageId() {
        T0A00TerminalRSAPublicKey message = new T0A00TerminalRSAPublicKey();
        assertEquals(MessageTypes.Terminal.TERMINAL_RSA_PUBLIC_KEY, message.getMessageId());
        assertEquals(0x0A00, message.getMessageId());
    }
    ...
    void testEncodeAndDecode() {
        ...
        T0A00TerminalRSAPublicKey originalMessage = new T0A00TerminalRSAPublicKey(exponent, modulus);
    ...
    void testToString() {
        ...
        assertTrue(result.contains("T0A00TerminalRSAPublicKey"));
        assertTrue(result.contains("messageId=0xA00"));
    ...
    void testEqualsAndHashCode() {
        ...
        T0A00TerminalRSAPublicKey message1 = new T0A00TerminalRSAPublicKey(exponent, modulus);
        T0A00TerminalRSAPublicKey message2 = new T0A00TerminalRSAPublicKey(exponent, modulus);
        T0A00TerminalRSAPublicKey message3 = new T0A00TerminalRSAPublicKey(3L, modulus);
    ...
    void testFactoryCreation() {
        ...
        assertInstanceOf(T0A00TerminalRSAPublicKey.class, message);
        assertEquals(0x0A00, message.getMessageId());
        
        // 测试消息是否被支持
        assertTrue(factory.isSupported(MessageTypes.Terminal.TERMINAL_RSA_PUBLIC_KEY));
        assertTrue(factory.getSupportedMessageIds().contains(MessageTypes.Terminal.TERMINAL_RSA_PUBLIC_KEY));
    }
    ...
    void testBigIntegerConversion() {
        ...
        T0A00TerminalRSAPublicKey message = new T0A00TerminalRSAPublicKey(BigInteger.valueOf(65537), largeModulus);
    ...
    void testEdgeCases() {
        ...
        T0A00TerminalRSAPublicKey decoded = new T0A00TerminalRSAPublicKey();
        decoded.decodeBody(encoded);
        assertEquals(message, decoded);
    }
    
    @Test
    void testRSAKeyInteroperability() {
        // 测试与平台RSA公钥消息的互操作性
        long exponent = 65537L;
        byte[] modulus = new byte[128];
        ...
        // 验证两个消息具有相同的RSA参数
        assertEquals(terminalKey.getPublicExponent(), platformKey.getPublicExponent());
        assertArrayEquals(terminalKey.getModulus(), platformKey.getModulus());
        assertEquals(terminalKey.getPublicExponentAsBigInteger(), platformKey.getPublicExponentAsBigInteger());
        assertEquals(terminalKey.getModulusAsBigInteger(), platformKey.getModulusAsBigInteger());
        
        // 验证编码格式相同
        Buffer terminalEncoded = terminalKey.encodeBody();
        Buffer platformEncoded = platformKey.encodeBody();
        assertEquals(terminalEncoded.length(), platformEncoded.length());
        assertArrayEquals(terminalEncoded.getBytes(), platformEncoded.getBytes());
    }
}